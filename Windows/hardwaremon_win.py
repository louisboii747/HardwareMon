...
class Sparkline(ctk.CTkCanvas):
    ...
    def _draw(self):
        ...
        if len(pts) >= 4:
            ...
            self.create_polygon(
                fill_pts, fill=self._hex_alpha(self.color, 0.15), outline="", smooth=True
            )
...
class ArcGauge(ctk.CTkCanvas):
    def __init__(self, master, size=80, **kw):
        super().__init__(master, width=size, height=size, bg=BG_CARD, highlightthickness=0, **kw)
        self._size = size
        self._value = 0.0
        self._target = 0.0
        self._after_id = None
    ...
    def _draw(self, pct: float, color: str):
        ...
        pad = 6
        self.create_arc(
            pad, pad, s - pad, s - pad, start=220, extent=-260, style="arc", outline=BORDER, width=5
        )
        extent = -260 * (pct / 100)
        if abs(extent) > 0.5:
            self.create_arc(
...
class MetricCard(ctk.CTkFrame):
    def __init__(
        self, master, title: str, icon: str, buf: RingBuffer, color=ACCENT2, unit="%", **kw
    ):
        super().__init__(
        ...
        ctk.CTkLabel(hdr, text=icon, font=("Segoe UI Emoji", 18), text_color=color).pack(
            side="left"
        )
        ctk.CTkLabel(
            hdr, text=title, font=("Segoe UI Variable", 12, "bold"), text_color=TEXT_PRI
        ...
        self.sub_label = ctk.CTkLabel(
            info_col, text="", font=("Segoe UI Variable", 10), text_color=TEXT_SEC
        )
...
class InfoPanel(ctk.CTkScrollableFrame):
    ...
    def set_section(self, title: str):
        ...
        ctk.CTkLabel(
            self, text=title, font=("Segoe UI Variable", 11, "bold"), text_color=ACCENT2
    ...
    def set_row(self, key: str, value: str, key_id: str = ""):
        ...
        lbl = ctk.CTkLabel(
            row, text=value, font=("Segoe UI Variable", 10, "bold"), text_color=TEXT_PRI, anchor="w"
        )
        lbl.pack(side="left")
        self._rows[rid] = lbl
...
class ProcessTable(ctk.CTkFrame):
    ...
    def refresh(self, procs):
        ...
        for i, p in enumerate(procs[:12]):
            ...
            vals = [name, str(p["pid"]), f"{p['cpu']:.1f}", f"{p['mem']:.0f}"]
            widths = [180, 55, 60, 70]
            colors = [TEXT_PRI, TEXT_SEC, severity_color(p["cpu"]), TEXT_PRI]
            for val, w, c in zip(vals, widths, colors):
                ctk.CTkLabel(
                    row, text=val, width=w, font=("Segoe UI Variable", 9), text_color=c, anchor="w"
...
class NavButton(ctk.CTkButton):
    ...
    def set_active(self, active: bool):
        if active:
            self.configure(
                fg_color="#0D1F33", text_color=ACCENT2, border_color=ACCENT, border_width=1
            )
        else:
...
class HardwareMonApp(ctk.CTk):
    ...
    def _build_layout(self):
        ...
        topbar = ctk.CTkFrame(self, fg_color=BG_CARD, corner_radius=0, border_width=0, height=48)
        ...
        self.time_label = ctk.CTkLabel(
            topbar, text="", font=("Segoe UI Variable", 10), text_color=TEXT_SEC
        )
        self.time_label.pack(side="right", padx=20)

        self.status_dot = ctk.CTkLabel(
            topbar, text="● LIVE", font=("Segoe UI Variable", 10, "bold"), text_color=GREEN
        )
        self.status_dot.pack(side="right", padx=8)

        self.sidebar = ctk.CTkFrame(
            self, fg_color=BG_SIDE, corner_radius=0, border_width=0, width=188
        )
    ...
    def _build_sidebar(self):
        ctk.CTkLabel(
            self.sidebar, text="NAVIGATION", font=("Segoe UI Variable", 9), text_color=TEXT_SEC
        ...
        for page in self.PAGES:
            btn = NavButton(
                self.sidebar, page, icons[page], command=lambda p=page: self._show_page(p)
            )
    ...
    def _build_overview(self):
        ...
        ctk.CTkLabel(
            p, text="Overview", font=("Segoe UI Variable", 20, "bold"), text_color=TEXT_PRI
        ...
        self.ov_disk = MetricCard(p, "Disk I/O", "💿", self.disk_buf, color=ORANGE, unit=" MB/s")
        self.ov_net = MetricCard(p, "Network", "🌐", self.net_buf, color=GREEN, unit=" KB/s")

        for i, card in enumerate([self.ov_cpu, self.ov_mem, self.ov_disk, self.ov_net]):
            card.grid(
                row=1,
                column=i,
                padx=(12 if i == 0 else 6, 6 if i < 3 else 12),
                pady=(0, 12),
                sticky="nsew",
            )

        self.proc_table = ProcessTable(p)
        self.proc_table.grid(row=2, column=0, columnspan=4, padx=12, pady=(0, 12), sticky="ew")
    ...
    def _build_cpu_page(self):
        ...
        ctk.CTkLabel(
            p, text="CPU Details", font=("Segoe UI Variable", 20, "bold"), text_color=TEXT_PRI
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=(16, 8))

        self.cpu_cores_frame = ctk.CTkFrame(
            p, fg_color=BG_CARD, corner_radius=12, border_width=1, border_color=BORDER
        )
        self.cpu_cores_frame.grid(row=1, column=0, padx=(12, 6), pady=(0, 12), sticky="nsew")

        ctk.CTkLabel(
            self.cpu_cores_frame,
            text="Per-Core Usage",
        ...
        for i in range(n):
            ...
            ctk.CTkLabel(
                cell, text=f"C{i}", width=24, font=("Segoe UI Variable", 9), text_color=TEXT_SEC
            ).pack(side="left")
            bar = ctk.CTkProgressBar(
                cell, height=8, corner_radius=4, fg_color=BORDER, progress_color=ACCENT2
            )
            bar.set(0)
            bar.pack(side="left", fill="x", expand=True, padx=4)
            pct_lbl = ctk.CTkLabel(
                cell, text="0%", width=32, font=("Segoe UI Variable", 9), text_color=TEXT_SEC
            )
    ...
    def _build_memory_page(self):
        ...
        ctk.CTkLabel(
            p, text="Memory", font=("Segoe UI Variable", 20, "bold"), text_color=TEXT_PRI
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=(16, 8))
        ...
        self.mem_info.grid(row=2, column=0, columnspan=2, padx=12, pady=(0, 12), sticky="ew")
    ...
    def _build_disk_page(self):
        ...
        ctk.CTkLabel(
            p, text="Disk", font=("Segoe UI Variable", 20, "bold"), text_color=TEXT_PRI
        ...
        self.disk_partitions_frame.grid(row=1, column=0, padx=12, pady=(0, 12), sticky="nsew")
    ...
    def _build_network_page(self):
        ...
        ctk.CTkLabel(
            p, text="Network", font=("Segoe UI Variable", 20, "bold"), text_color=TEXT_PRI
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=(16, 8))

        # FIX: use dedicated buffers so sparklines accumulate real history
        self.net_send_card = MetricCard(
            p, "Upload", "⬆️", self.net_send_buf, color=ORANGE, unit=" KB/s"
        )
        self.net_recv_card = MetricCard(
            p, "Download", "⬇️", self.net_recv_buf, color=GREEN, unit=" KB/s"
        )
        self.net_send_card.grid(row=1, column=0, padx=(12, 6), pady=(0, 12), sticky="nsew")
        self.net_recv_card.grid(row=1, column=1, padx=(6, 12), pady=(0, 12), sticky="nsew")

        self.net_info = InfoPanel(p)
        self.net_info.grid(row=2, column=0, columnspan=2, padx=12, pady=(0, 12), sticky="ew")
    ...
    def _collect_and_update(self):
        ...
        if self._thread_tick >= 5:
            try:
                self._thread_count = sum(p.num_threads() for p in psutil.process_iter())
            except Exception:
                pass
            self._thread_tick = 0
    ...
    def _update_ui(
        self, cpu_pct, core_pcts, mem_pct, vm, sw, sent, recv, net, procs, parts, freq, disk_mb_s
    ):
        ...
        self.ov_mem.update_data(
            mem_pct, f"{self._fmt_bytes(vm.used)} / {self._fmt_bytes(vm.total)}"
        )
        # FIX: show actual MB/s rate; clamp gauge to 100 for display only
        self.ov_disk.update_data(min(disk_mb_s, 100.0), f"{disk_mb_s:.2f} MB/s")
        self.ov_net.update_data(min(recv, 1000), f"↑ {sent:.0f}  ↓ {recv:.0f} KB/s")
        ...
        for i, (bar, lbl) in enumerate(self._core_bars):
            if i < len(core_pcts):
                ...
                lbl.configure(text=f"{core_pcts[i]:.0f}%", text_color=severity_color(core_pcts[i]))
        ...
        self.swap_card.update_data(
            sw.percent, f"{self._fmt_bytes(sw.used)} / {self._fmt_bytes(sw.total)}"
        )
        ...
        if self._disk_parts_tick == 0:
            ...
            for part in parts:
                ...
                ctk.CTkLabel(
                    hdr,
                    text=f"💿  {part['device']}  ({part['fstype']})",
                    font=("Segoe UI Variable", 11, "bold"),
                    text_color=TEXT_PRI,
                ).pack(side="left")
                ctk.CTkLabel(
                    ...
                    text=f"{part['pct']:.1f}%",
                    font=("Segoe UI Variable", 11, "bold"),
                    text_color=severity_color(part["pct"]),
                ).pack(side="right")
                bar = ctk.CTkProgressBar(
                    card,
                    height=6,
